...
public class AVTransportService extends AbstractService { // implements ServiceEventHandler
     ...
     */
    public int addToQueue(final Entry entry, final int p) {
        int position = p;
        if (position < 0) {
            position = -1;
        }
        ...
        // message.setInput("EnqueuedURIMetaData", "");
        // message.setInput("DesiredFirstTrackNumberEnqueued", String.valueOf(position + 1));
        message.setInput("DesiredFirstTrackNumberEnqueued", "0");
        message.setInput("EnqueueAsNext", "1");
        executeImmediate(message);
        final String output = message.getOutputAsString("FirstTrackNumberEnqueued");
    ...
    public PositionInfo getPositionInfo() {
        ...
        try {
            final String metaDataString = state.get(AVTransportEventType.CurrentTrackMetaData);
            trackMetaData = metaDataString.length() == 0 ? null : ResultParser.parseTrackMetaData(metaDataString);
        } catch (Exception e) {
        }
     ...
     *            @ * Signals that an I/O exception has occurred.
     */
    public void removeTrackFromQueue(final int position) {
        SonosActionInvocation message = messageFactory.getMessage(getService(), "RemoveTrackFromQueue");
        message.setInput("InstanceID", "0");
        message.setInput("ObjectID", "Q:0/" + position);
        // message.setInput("UpdateID", "0");
        execute(message);
    }
